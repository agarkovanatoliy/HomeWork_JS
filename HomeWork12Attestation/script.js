...
function delProduct(product) {
  let total = +cardToBascet.querySelector(".basket__content-total-sum")
    .textContent;
...
const rangeInput = document.querySelectorAll(
  ".contentProduct__catalog-product-sort-range-input input"
);
const priceInput = document.querySelectorAll(
  ".contentProduct__catalog-product-sort-price-input input"
);
const range = document.querySelector(
  ".contentProduct__catalog-product-sort-slider .contentProduct__catalog-product-sort-progress"
);
let priceGap = 50;
...
priceInput.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (maxPrice - minPrice >= priceGap && maxPrice <= rangeInput[1].max) {
      if (
        e.target.className ===
        "contentProduct__catalog-product-sort-price-input-min"
      ) {
...
rangeInput.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (maxVal - minVal < priceGap) {
      if (
        e.target.className === "contentProduct__catalog-product-sort-range-min"
      ) {
